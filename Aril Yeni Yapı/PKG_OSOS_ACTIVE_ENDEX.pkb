...
-------------------------------------------------------------------------------
  ...
  --  Description    : Golden Gate ?zerinden al?nan datalar birle?tiriliyor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
  ...
  --  Description    : Aril uygulamas?n?n endeksleri al?n?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    ...
    v_src_table_03  varchar2(30) := 'GG_SDM_VAL_DEFINITIONITEMS';
  ...
  BEGIN
    ...
    gv_dyn_task := '
      ...
      SELECT /*+PARALLEL(E,128) PARALLEL(D,32) PARALLEL(M,32) */ 
            TO_NUMBER(E.OWNERSERNO) SUBSCRIBER_APP_ID,
            TO_NUMBER(D.IDENTIFIERVALUE) SUBSCRIBER_ID,
            TO_NUMBER(E.SENSORSERNO) METER_APP_ID,
            ...
            TO_NUMBER(M.MULTIPLIER) MULTIPLIER,
            TO_NUMBER(E.TSUM * M.MULTIPLIER) TOTAL_ENDEX,
            TO_NUMBER(E.T1ENDEX * M.MULTIPLIER) T1_ENDEX,
            TO_NUMBER(E.T2ENDEX * M.MULTIPLIER) T2_ENDEX,
            TO_NUMBER(E.T3ENDEX * M.MULTIPLIER) T3_ENDEX,
            TO_NUMBER(E.T4ENDEX * M.MULTIPLIER) T4_ENDEX,
            TO_NUMBER(E.REACTIVECAPASITIVE) REACTIVE_CAPASITIVE_ENDEX,
            TO_NUMBER(E.REACTIVEINDUCTIVE) REACTIVE_INDUCTIVE_ENDEX,
            TO_NUMBER(E.MAXDEMAND) MAX_DEMAND,
            TO_CHAR(E.DEMANDDATE) DEMAND_DATE,
            TO_NUMBER(E.ENDEXTYPE) ENDEX_TYPE,
            TO_NUMBER(E.RECORDTYPE) RECORD_TYPE,
            1 APPLICATION_ID
      FROM '||gv_stg_owner||'.'||v_src_table_01||' E 
      INNER JOIN '||gv_ods_owner||'.'||v_src_table_02||' D ON (E.OWNERSERNO = D.SERNO) AND D.DEFINITIONTYPESERNO IN( 2,9,11)
      INNER JOIN '||gv_ods_owner||'.'||v_src_table_03||' M ON (E.SENSORSERNO = M.SERNO) AND M.DEFINITIONTYPESERNO = 4
    ...
    MODIFY(DEMAND_DATE VARCHAR2(40 BYTE))
    ';
  ...
  --  Description    : Hayen uygulamas?n?n endeksleri al?n?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
  ...
  BEGIN
    ...
    gv_dyn_task := '
      ...
      AS      
      SELECT /*+PARALLEL(128) */
                TO_NUMBER(SUBSCRIBER_APP_ID) SUBSCRIBER_APP_ID,
                TO_NUMBER(SUBSCRIBER_ID) SUBSCRIBER_ID,
                TO_NUMBER(METER_APP_ID) METER_APP_ID,
                TO_NUMBER(METER_ID) METER_ID,
                ENDEX_DATETIME,
                ENDEX_DATE,
                TO_NUMBER(MULTIPLIER) MULTIPLIER,
                TO_NUMBER(TOTAL_ENDEX) TOTAL_ENDEX,
                TO_NUMBER(T1_ENDEX) T1_ENDEX,
                TO_NUMBER(T2_ENDEX) T2_ENDEX,
                TO_NUMBER(T3_ENDEX) T3_ENDEX,
                TO_NUMBER(T4_ENDEX) T4_ENDEX,
  ...
  --  Description    : Luna uygulamas?n?n endeksleri al?n?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
  ...
  BEGIN
    ...
    gv_dyn_task := '
      ...
      AS      
      SELECT /*+PARALLEL(128) */ 
        TO_NUMBER(ABONE_ID) SUBSCRIBER_APP_ID,                       
        TO_NUMBER(ABONE_NO) SUBSCRIBER_ID,
        TO_NUMBER(SAYAC_ID) METER_APP_ID,
        TO_NUMBER(SAYAC_NO) METER_ID,
        OKUMATARIHSAAT ENDEX_DATETIME,
        OKUMATARIH ENDEX_DATE,
        TO_NUMBER(SAYACCARPANI) MULTIPLIER,
        TO_NUMBER(TNUMERIK) TOTAL_ENDEX,
        TO_NUMBER(T1NUMERIK) T1_ENDEX,
        TO_NUMBER(T2NUMERIK) T2_ENDEX,
        TO_NUMBER(T3NUMERIK) T3_ENDEX,
        TO_NUMBER(T4NUMERIK) T4_ENDEX,
        0 REACTIVE_CAPASITIVE_ENDEX,
        0 REACTIVE_INDUCTIVE_ENDEX,
        0 MAX_DEMAND,
        LPAD('' '',40,8) DEMAND_DATE,
        0 ENDEX_TYPE,
        1 RECORD_TYPE,
        2 APPLICATION_ID 
      FROM
  ...
  --  Description    : Uygulamalara ait olan endeks bilgileri birle?tiriliyor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    ...
    v_src_table_02  varchar2(30) := 'T_LUNA_ACTIVE_ENDEX';
    v_src_table_03  varchar2(30) := 'T_HAYEN_ACTIVE_ENDEX';
    ----------------------------------------------------------------------------
  BEGIN
  ...
  --  Description    : Endekslerin bir ?nceki de?eleri bulunuyor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
...
PROCEDURE PRC_BACK_ENDEX_HAYEN
  ...
  --  Creation Date  : 03-11-2017
  --  Last Modification Date  : 03-11-2017
  --  Procedure Name : PRC_BACK_ENDEX_HAYEN
  --  Description    : Endekslerin bir ?nceki de?eleri bulunuyor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    v_table_name    varchar2(35) := 'T_BACK_ENDEX_HAYEN';
    ----------------------------------------------------------------------------
    v_src_table_01  varchar2(30) := 'T_HAYEN_ACTIVE_ENDEX';
  BEGIN
    gv_proc := 'PRC_BACK_ENDEX';
  ...
  --  Description    : Endeksler ?zerinden t?ketim hesaplan?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
  ...
  --  Creation Date  : 03-11-2017
  --  Last Modification Date  : 03-11-2017
  --  Procedure Name : T_CONSUMPTION_ACTIVE_ENDEX_H
  --  Description    : Endeksler ?zerinden t?ketim hesaplan?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    v_table_name    varchar2(35) := 'T_CONSUMPTION_ACTIVE_ENDEX_H';
    ----------------------------------------------------------------------------
    v_src_table_01  varchar2(30) := 'T_BACK_ENDEX_HAYEN';
    ----------------------------------------------------------------------------
  BEGIN
    gv_proc := 'PRC_CONSUMPTION_ACTIVE_ENDEX_H';
  ...
  --  Description    : Endeks t?ketimleri ve endeks de?erleri tabloya bas?l?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    ...
    v_src_table_02  varchar2(30) := 'T_CONSUMPTION_ACTIVE_ENDEX_H';
    ----------------------------------------------------------------------------
  BEGIN
    ...
    PL.EXCHANGE_PARTITION(gv_stg_owner,v_table_name,'APP_CONS',gv_stg_owner||'.'||v_src_table_01);

--    PL.EXCHANGE_PARTITION(gv_stg_owner,v_table_name,'APP_ARIL_LUNA',gv_stg_owner||'.'||v_src_table_01);
--
--    PL.EXCHANGE_PARTITION(gv_stg_owner,v_table_name,'APP_HAYEN',gv_stg_owner||'.'||v_src_table_02);
  ...
  --  Description    : K2 abone listesi al?n?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
  ...
  --  Description    : K2 abonelerinin t?ketimleri hesaplan?p tabloya bas?l?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
  ...
  BEGIN
    ...
    PL.DROP_TABLE(gv_stg_dm_owner, v_table_name);
    ...
    gv_dyn_task := '      
      ...
      WHERE REPORT_NAME = ''K2 Osos T?ketimi Endeks Bilgileri- Oracle'' AND ID =161
    ';
    PL.LOGGER.SUCCESS(SQL%ROWCOUNT,gv_dyn_task);
  ...
  --  Description    : Tar?msal t?ketimler hesaplan?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
...
--    INTO '||gv_stg_dm_owner||'.'||v_table_name||'
    SELECT /* +PARALLEL(E,128)*/
        M.MBS_PROVINCE,
        E.ENDEX_DATE,
  ...
  --  Description    : Tar?msal t?ketimi hesaplanan abone say?lar? belirleniyor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
...
--    INTO '||gv_stg_dm_owner||'.'||v_table_name||'
  ...
  --  Description    : YKP den al?nan saatlik t?ketim verileri g?nl?k t?ketime d?n??t?r?l?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
...
--    INTO '||gv_stg_owner||'.'||v_table_name||'
  ...
  --  Description    : Hesaplanan g?nl?k t?ketim verileri ayl?k t?ketime d?n??t?r?l?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
...
--    INTO '||gv_stg_owner||'.'||v_table_name||'
  ...
  --  Description    : Tar?msal t?ketim verileri crosstab e d?n??t?r?l?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
...
--    PL.DROP_TABLE(gv_stg_dm_owner, v_table_name);
    ...
    gv_dyn_task := '
    CREATE TABLE '||gv_stg_owner||'.'||v_table_name||' PARALLEL 128 NOLOGGING COMPRESS AS
--    INSERT --+PARALLEL(128) APPEND NOLOGGING
--    INTO '||gv_stg_dm_owner||'.'||v_table_name||'
    SELECT
  ...
  --  Description    : Ayd?nlatma t?ketimleri hesaplan?p tabloya yaz?l?yor.
  --  [Modification History]
  ------------------------------------------------------------------------------
    ----------------------------------------------------------------------------
